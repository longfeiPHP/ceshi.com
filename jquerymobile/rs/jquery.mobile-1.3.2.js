...
(function ( root, doc, factory ) {//三个参数 window/document/function 
	if ( typeof define === "function" && define.amd ) {//暂时不考虑
		...
		// Browser globals
		factory( root.jQuery, root, doc );//jquery/window/document
	}
}( this, document, function ( jQuery, window, document, undefined ) {
/**
 * 第一个方法 给jquery对象添加一个mobile属性,初始化为空对象
 */
(function( $ ) {
	$.mobile = {};
}( jQuery ));
...
(function( $, window, undefined ) {//jquery/window
	var nsNormalizeDict = {};//定义一个空对象  状态:未知

	// jQuery.mobile configurable options 配置选项
	$.mobile = $.extend($.mobile, {
		...
		subPageUrlKey: "ui-page",//状态:未知

		// Class assigned to page currently in view, and during transitions
		activePageClass: "ui-page-active",//当前页的class

		// Class used for "active" button state, from CSS framework
		activeBtnClass: "ui-btn-active",

		// Class used for "focus" form element state, from CSS framework
		focusClass: "ui-focus",//input标签默认会在外层加上一个div,当聚焦该input的时候会在那个div上面加上这个class名

		// Automatically handle clicks and form submissions through Ajax, when same-domain
		ajaxEnabled: true,//同域下是否使用ajax提交

		// Automatically load and show pages based on location.hash
		hashListeningEnabled: true,//本地加载

		// disable to prevent jquery from bothering with links
		linkBindingEnabled: true,//状态:未知

		// Set default page transition - 'none' for no transitions
		defaultPageTransition: "fade",//设置过渡页的显示方式

		// Set maximum window width for transitions to apply - 'false' for no limit
		maxTransitionWidth: false,//状态:未知

		// Minimum scroll distance that will be remembered when returning to a page
		minScrollBack: 250,//后退多少步

		// DEPRECATED: the following property is no longer in use, but defined until 2.0 to prevent conflicts
		touchOverflowEnabled: false,//虽然该属性不用了,但是在2.0之前还会保留

		// Set default dialog transition - 'none' for no transitions
		defaultDialogTransition: "pop",//弹窗默认效果

		// Error response message - appears when an Ajax page request fails
		pageLoadErrorMessage: "Error Loading Page",//页面错误提示

		// For error messages, which theme does the box uses?
		pageLoadErrorMessageTheme: "e",//错误提示样式

		// replace calls to window.history.back with phonegaps navigation helper
		// where it is provided on the window object
		phonegapNavigationEnabled: false,//状态:未知

		//automatically initialize the DOM when it's ready
		autoInitializePage: true,//自动初始化DOM

		pushStateEnabled: true,//推送状态

		// allows users to opt in to ignoring content by marking a parent element as
		// data-ignored
		ignoreContentEnabled: false,//状态:未知

		// turn of binding to the native orientationchange due to android orientation behavior
		orientationChangeEnabled: true,//状态:未知
		...
		// Scroll page vertically: scroll to 0 to hide iOS address bar, or pass a Y value 垂直滚动
		silentScroll: function( ypos ) {//成员方法
			if ( $.type( ypos ) !== "number" ) {
